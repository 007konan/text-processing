import java.io.*;
import java.util.*;

public class Search {
    
    public static Map<Integer,Double> similarity(String query , String[] profile) {
	Map<Integer,Double> sim = new TreeMap<>();
	try {
	    File file = new File("tfidf100.csv");
	    BufferedReader br = new BufferedReader(new FileReader(file));
       	    
	    String line = "";
	    line = br.readLine();
	    String[] num = line.split(",",0);
	    int N = Integer.parseInt(num[0]);    //文書数
	    int term = Integer.parseInt(num[1]); //索引語数
	 
	    String[] termList = new String[term];
	    
	    int term_num = 0;
	    Map<Integer,TreeMap<Integer,Double>> link = new TreeMap<>();
	    while ((line = br.readLine()) != null) {
		String[] data = line.split(",",0);		
		termList[term_num] = data[0];

		for (int i = 1; i < data.length; i++){
		    String[] data2 = data[i].split(" ",0);
		    TreeMap<Integer,Double> tmap = link.get(Integer.parseInt(data2[0]));
		    if (tmap == null) {
			tmap = new TreeMap<Integer,Double>();
		    }
		    tmap.put(term_num , Double.parseDouble(data2[1]));
		    link.put(Integer.parseInt(data2[0]) , tmap);
		}
		term_num++;
	    }
		    
	    br.close();

	    // 検索質問のベクトルを求める
	    // 質問が索引語と一致したらリストに1をいれる
	    ArrayList<String> searchQuestions = new ArrayList<>();
	    double alfa = 0.1;
	    for (int i = 0; i < term; i++){
		String str = termList[i];
		for (int j = 0; j < profile.length; j++) {
		    if (str.equals(profile[j])) {
			searchQuestions.add(i+" "+alfa*1);
		    }
		}
		if (str.equals(query)) {
		    searchQuestions.add(i+" "+1);
		}
	    }
	    
	    // 記事と検索質問の類似度の計算
	    double document_w = 0;
	    double question_w = 0;
	    double question_document = 0;
	    for (int i = 0; i < searchQuestions.size(); i++) {
		String question_v1 = searchQuestions.get(i);
		String[] qn1 = question_v1.split(" ",0);
		question_w += Double.parseDouble(qn1[1]) * Double.parseDouble(qn1[1]);
	    }
	    question_w = Math.sqrt(question_w);

	    for (int j = 0; j < N; j++) {
		document_w = 0;
		question_document = 0;

		TreeMap<Integer,Double> tmap = link.get(j+1);
		if (tmap != null) {
		    for (Map.Entry<Integer,Double> entry : tmap.entrySet()){
			document_w += entry.getValue() * entry.getValue();
			for (int k = 0; k < searchQuestions.size(); k++) {
			    String question_v2 = searchQuestions.get(k);
			    String[] qn2 = question_v2.split(" ",0);
			    if (Integer.parseInt(qn2[0]) == entry.getKey()) {
				question_document += Double.parseDouble(qn2[1]) * entry.getValue();
			    }
			}		    
		    }
		    document_w = Math.sqrt(document_w);
		    // cosが0以外をマップに格納
		    if (question_document != 0) {
			sim.put(j+1,question_document / (question_w * document_w));
		    }
		}
	    }
	   	    
	} catch (FileNotFoundException e) {
	    System.out.println(e);
	} catch (IOException e) {
	    System.out.println(e);
	}
	return sim;
    }

    // 関連記事の類似度計算
    public static Map<Integer,Double> similarity2(int dn , String[] profile) {
	Map<Integer,Double> sim2 = new TreeMap<>();
	try {
	    File file = new File("tfidf100.csv");
	    BufferedReader br = new BufferedReader(new FileReader(file));
       	    
	    String line = "";
	    line = br.readLine();
	    String[] num = line.split(",",0);
	    int N = Integer.parseInt(num[0]);    //文書数
	    int term = Integer.parseInt(num[1]); //索引語数
	 
	    String[] termList = new String[term];
	    
	    int term_num = 0;
	    Map<Integer,TreeMap<Integer,Double>> link = new TreeMap<>();
	    while ((line = br.readLine()) != null) {
		String[] data = line.split(",",0);		
		termList[term_num] = data[0];

		for (int i = 1; i < data.length; i++){
		    String[] data2 = data[i].split(" ",0);
		    TreeMap<Integer,Double> tmap = link.get(Integer.parseInt(data2[0]));
		    if (tmap == null) {
			tmap = new TreeMap<Integer,Double>();
		    }
		    tmap.put(term_num , Double.parseDouble(data2[1]));
		    link.put(Integer.parseInt(data2[0]) , tmap);
		}
		term_num++;
	    }
	    br.close();

	    // 閲覧中の記事のベクトルを求める
	    ArrayList<String> searchQuestions = new ArrayList<>();
	    double alfa = 0.1;
	    TreeMap<Integer,Double> doc = link.get(dn);
	    for (int i = 0; i < term; i++){
		String str = termList[i];
		for (int j = 0; j < profile.length; j++) {
		    if (str.equals(profile[j])) {
			searchQuestions.add(i+" "+alfa*1);
		    }
		}
		if (doc.get(i) != null) {
		    searchQuestions.add(i+" "+doc.get(i));
		}
	    }
	    // 記事と検索質問の類似度の計算
	    double document_w = 0;
	    double question_w = 0;
	    double question_document = 0;

	    
	    for (int i = 0; i < searchQuestions.size(); i++) {
		String question_v1 = searchQuestions.get(i);
		    String[] qn1 = question_v1.split(" ",0);
		    question_w += Double.parseDouble(qn1[1]) * Double.parseDouble(qn1[1]);
	    }
	    question_w = Math.sqrt(question_w);
	    
	    for (int j = 0; j < N; j++) {
		document_w = 0;
		question_document = 0;

		TreeMap<Integer,Double> tmap = link.get(j+1);
		if (tmap != null) {
		    for (Map.Entry<Integer,Double> entry : tmap.entrySet()){
			document_w += entry.getValue() * entry.getValue();
			for (int k = 0; k < searchQuestions.size(); k++) {
			    String question_v2 = searchQuestions.get(k);
			    String[] qn2 = question_v2.split(" ",0);
			    if (Integer.parseInt(qn2[0]) == entry.getKey()) {
				question_document += Double.parseDouble(qn2[1]) * entry.getValue();
			    }
			}		    
		    }
		    document_w = Math.sqrt(document_w);
		    // cosが0以外をマップに格納
		    if (question_document != 0 && j+1 != dn) {
			sim2.put(j+1,question_document / (question_w * document_w));
		    }
		}
	    }
	} catch (FileNotFoundException e) {
	    System.out.println(e);
	} catch (IOException e) {
	    System.out.println(e);
	}
	return sim2;
    }

    // トップn記事のタイトルを探す
    public static int[] searchTitle(int[] num) {
	int[] textnumber = new int[num.length];
	try {
	    File file = new File("title100.txt");
	    BufferedReader br = new BufferedReader(new FileReader(file));
       	    
	    // タイトル検索
	    int count = 1;  // 文書番号
	    String[] title = new String[num.length];
	    String line = "";
	    while ((line = br.readLine()) != null) {
		for (int i = 0; i < num.length; i++) {
		    if (count == num[i]){
			title[i] = line;
			textnumber[i] = count;
		    }
		}
		count++;
	    }

	    // トップn記事の出力
	    System.out.println("検索結果TOP" + num.length);
	    for (int i = 0; i < title.length; i++) {
		if (title[i] == null) {
		    System.out.println("これ以上見つかりませんでした。");
		    break;
		}
		System.out.println((i+1) + ":" + title[i]);
	    }
	    br.close();
	} catch (FileNotFoundException e) {
	    System.out.println(e);
	} catch (IOException e) {
	    System.out.println(e);
	}
	return textnumber;
    }

    // 類似度の高い順にソート	
    public static int[] sort(Map<Integer,Double> map , int n) {
	List<Map.Entry<Integer,Double>> entries = new ArrayList<Map.Entry<Integer,Double>>(map.entrySet());
	
	Collections.sort(entries, new Comparator<Map.Entry<Integer,Double>>() {
		
		public int compare(Map.Entry<Integer,Double> entry1 , Map.Entry<Integer,Double> entry2) {
		    return ((Double)entry2.getValue()).compareTo((Double)entry1.getValue());
		}
	    });

	int x = 0;
	int[] list = new int[n];
	for(Map.Entry<Integer,Double> entry : entries){
	    if (x < n){
		list[x] = entry.getKey();
	    }
	    x++;
	}
	return list;
    }

    //本文出力
    public static void text(int i) {
	try {
	    File file = new File("wiki100.txt");
	    BufferedReader br = new BufferedReader(new FileReader(file));
       	    
	    int dn = 1;
	    String line = "";
	    while ((line = br.readLine()) != null) {
	       	if (line.contains("</doc>")) {
		    dn++;
		    line = br.readLine();
		}
		else if (dn == i) {
		    System.out.println(line);
		}
	    }

	    br.close();
	} catch (FileNotFoundException e) {
	    System.out.println(e);
	} catch (IOException e) {
	    System.out.println(e);
	}
    }

    public static void main(String[] args) {
	Scanner stdIn = new Scanner(System.in);

	int n = 10;   // n記事を返す
	int m = 5;    // m個の関連記事
	
	Map<Integer,Double> sim = new TreeMap<>();
	Map<Integer,Double> sim2 = new TreeMap<>();
	int[] list = new int[n];
	int[] textnumber = new int[n];

	String[] profile = {"阪神","広島","ヤクルト","DeNA","中日","巨人"};

	int a=0;
	do {
	    System.out.print("検索質問を入力してください:");
	    String query = stdIn.next();
	
	    // 類似度計算
	    sim = similarity(query,profile);
	
	    list = sort(sim,n);
	    // 記事のタイトル検索
	    textnumber = searchTitle(list);
	    
	    System.out.print("記事:");
	    a = stdIn.nextInt();
	    while (a < 1 || a > n) {
		System.out.print("1〜" + n + "を入力してください:");
		a = stdIn.nextInt();
	    }
	    
	    do {
		text(textnumber[a-1]);
		sim2 = similarity2(textnumber[a-1],profile);
	    
		list = sort(sim2,m);
		// 記事のタイトル検索
		System.out.print("関連記事の");
		textnumber = searchTitle(list);
		
		System.out.println();
		
		System.out.println((m+1) +":別の語を検索\n" + (m+2) + ":終わる");
		System.out.print("どうしますか:");
		a = stdIn.nextInt();
	
		while (a < 1 || a > m+2) {
		    System.out.print("1〜" + (m+2) + "を入力してください:");
		    a = stdIn.nextInt();
		}
	    } while(a > 0 && a <= m);
	} while(a == (m+1));
	
    }
}
